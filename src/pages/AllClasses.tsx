...
import { getApprovedClasses, getAllClasses } from '@/services/classService';
...
const AllClasses = () => {
  ...
  const { data: classes = [], isLoading } = useQuery({
    queryKey: ['approved-classes', debouncedSearchQuery],
    queryFn: () => {
      if (debouncedSearchQuery) {
        // Filter classes based on search query
        return getAllClasses().then(data => 
          data.filter((cls: any) => 
            cls.title.toLowerCase().includes(debouncedSearchQuery.toLowerCase()) ||
            cls.name.toLowerCase().includes(debouncedSearchQuery.toLowerCase()) ||
            cls.category.toLowerCase().includes(debouncedSearchQuery.toLowerCase())
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <div className="space-y-6">
            <div>
              <h1 className="text-4xl font-bold mb-4 text-gradient">All Classes</h1>
              <p className="text-lg text-muted-foreground">Discover and enroll in amazing courses</p>
            </div>
